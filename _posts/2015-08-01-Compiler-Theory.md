...
补充一下：在合并中可能有**规约-规约**冲突，但是不会出现**规约-移入**冲突（不然合并前的语法中也是有冲突的）。

从LR(1)合并得到LALR(1)需要较大的内存，另一种高效的思路是从LR(0)添加向前看符号，添加途径有两种：

1. 自发
2. 传播

有一个简单办法检测向前看符号是哪种方式生成的：令#为一个不在当前文法中的符号，对于LR(0)项集I的内核K以及一个文法符号X，判断方法如下：
...
有很多方法可以实现传播过程，从某种意义上说，所有这些方法都跟踪已经传播到某个项但尚未传播出去的“新”的向前看符号，下面是一种方法（输入增广文法G）：

1. 构造G的LR(0)项集族的内核
2. 将上面的检测算法应用于每个LR(0)项集的内核和每个文法符号X，确定GOTO(I,X)中各内核项的哪些向前看符号号是自发生成的，并确定向前看符号从I中的哪个项被传播到GOTO(I,X)中的内核项上
3. 初始化一个表格，表中给出每个项集中每个内核项相关的向前看符号。最初，每个项的向前看符号只包括那些步骤(2)中确定为自发生成的符号
4. 不断扫描所有项集的内核项，当访问项i时，使用步骤(2)中得到的、表格的信息，确定i将它的向前看符号传播到了哪些内核项中。项i的向前看符号集合被加到和这些被传播的内核项相关联的向前看符号集合中。我们继续扫描内核项知道么有心的向前看符号被传播为止
